  ...
  while (r.status !== LoginStatus.Ok) {
    if (r.status === LoginStatus.TwoFactor) {
      ...
      if (!process.env.EUFY_2FA) {
        console.error(`[eufy-bridge] 2FA code sent via ${r.method ?? "account default"} — set EUFY_2FA and redeploy.`);
        process.exit(1);
      }
      r = await eufy.submitVerifyCode(process.env.EUFY_2FA);
    } else if (r.status === LoginStatus.Captcha) {
      console.error("[eufy-bridge] eufy is asking for a captcha — this needs a manual run to clear, see feasibility test.");
      process.exit(1);
    } else {
...
boot()
  .then(() => app.listen(PORT, () => console.log(`[eufy-bridge] listening on :${PORT}`)))
  .catch((e) => {
    console.error("[eufy-bridge] FATAL boot error:", e instanceof Error ? e.message : e);
    process.exit(1);
  });
