...
test('WhatsApp video is a seekable playable H264 MP4; invalid input fails', async () => {
 ...
 const source=spawnSync(ffmpeg,['-hide_banner','-loglevel','error','-f','lavfi','-i','color=c=blue:s=160x90:r=15','-t','1','-c:v','libx264','-movflags','frag_keyframe+empty_moov','-f','mp4','pipe:1']);
 assert.equal(source.status,0);
 const output=await whatsappVideo(source.stdout);
 ...
 assert.equal(decoded.status,0);assert.match(decoded.stderr.toString(),/Video: h264/);
